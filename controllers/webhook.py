...
class ZadarmaWebhook(http.Controller):
    ...
    def _find_partner(self, norm_phone):
        ...
        return (
            request.env['res.partner'].sudo().browse(row[0])
            if row
            else request.env['res.partner'].sudo().browse()
        )
    ...
    def _find_user_by_sip(self, sip):
        ...
        return (
            ...
            .sudo()
            .search([('zadarma_internal_number', '=', str(sip).strip())], limit=1)
        )
    ...
    def _post_chatter(
        ...
        body = Markup(
            ...
            direction=direction_label, phone=phone, duration=duration_str, status=status or '—'
        )
        if recording_url:
    ...
    def _zadarma_get_recording_url(self, call_id_with_rec, pbx_call_id):
        company = (
            request.env['res.company'].sudo().search([('zadarma_api_key', '!=', False)], limit=1)
        )
        if not company:
    ...
    def _process_notify_record(self, data):
        ...
        try:
            ...
            _logger.info(
                'Zadarma: Downloaded and saved recording for call %s (%d bytes)',
                pbx_call_id,
                len(resp.content),
            )
        except Exception as e:
