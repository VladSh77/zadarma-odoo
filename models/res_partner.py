...
class ResPartner(models.Model):
    ...
    def action_zadarma_call(self):
        ...
        target = re.sub(r'\D', '', str(self.phone or self.mobile or ''))

        _logger.info("ZADARMA DATA: Key_exists=%s, Secret_exists=%s, SIP='%s', Target_Phone='%s'",
                     bool(key), bool(secret), sip_id, target)

        if not (key and secret):
            raise UserError("Налаштуйте API ключі Zadarma у налаштуваннях компанії.")
        if not sip_id:
            raise UserError("Вкажіть внутрішній SIP номер у профілі користувача.")
        if not target:
            raise UserError("У контакта не вказано номер телефону.")
        ...
        try:
            ...
            raise UserError(f"Помилка з'єднання з Zadarma: {str(e)}")
